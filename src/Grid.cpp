...
void Grid::doStep(int x, int y, Color color) {
    ...
    data[y][x] = color;
    updateRoad(x, y, ((color == WHITE) ? 7 : 1));
}
...
void Grid::doMove(Move move, Color color) {
    ...
    updateRoad(move.x0, move.y0, ((color == WHITE) ? 7 : 1));

    if (move.x1 == -1) return;
    data[move.y1][move.x1] = color;
    updateRoad(move.x1, move.y1, ((color == WHITE) ? 7 : 1));
}


void Grid::updateRoad(int x, int y, int weight) {
    // 纵向
